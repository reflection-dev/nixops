...
Typical contents (all three scaffolded by `add-host`):

- **`default.nix`** -- the host's own module. Imports the two
  below; sets host-specific services and options. Edited by hand.
- **`disko.nix`** -- disk layout. Either copied from a preset
  during `add-host` (e.g. `hetzner-vm`) or written by you for
  `custom`. See [Host files: disko, hardware-config, default.nix](host-files.md).
- **`hardware-configuration.nix`** -- kernel modules and hardware
  quirks. **Generated by `install-host`** on first install via
  `nixos-anywhere --generate-hardware-config`. `add-host` leaves a
  placeholder that is overwritten. Do not hand-edit.

Example `hosts/web-1/default.nix` after you have added services:
